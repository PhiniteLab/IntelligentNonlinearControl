...
class FLNNControllerAugNoPE:
    def __init__(self, dt, device, dtype, initial_weight=None):
        ...
        self.device = device
        self.dtype = dtype
        # Controller gains as per MATLAB code
        self.Kv = 50 * torch.eye(2, dtype=dtype, device=device)
        self.lam = 10 * torch.eye(2, dtype=dtype, device=device)
        self.F_rate = 50.0  # learning rate
        self.kappa = 5.0    # additional gain for weight decay term
    ...
    def step(self, x_des, x):
        """
        Compute control input using the FLNN method (MATLAB version).
        ...
        """
        # Extract desired states and current state
        qd   = x_des[0:2]
        qdp  = x_des[2:4]
        qdpp = x_des[4:6]
        q    = x[0:2]
        ...
        # x_m_q: based on zeta1 and q: [zeta1, zeta1*sin(q), zeta1*cos(q)]
        x_m_q = torch.cat([zeta1, 
                           zeta1 * torch.sin(q), 
                           zeta1 * torch.cos(q)])
        # x_v_m: based on zeta2, q, and q_dot: [zeta2*sin(q)*q_dot, zeta2*cos(q)*q_dot]
        x_v_m = torch.cat([zeta2 * torch.sin(q) * q_dot, 
                           zeta2 * torch.cos(q) * q_dot])
        # x_g_q: based on q: [cos(q), sin(q), sin(q)*cos(q), cos(q)*cos(q)]
        x_g_q = torch.cat([torch.cos(q), 
                           torch.sin(q), 
                           torch.sin(q) * torch.cos(q), 
                           torch.cos(q) * torch.cos(q)])
        # x_f_qd: based on q_dot: [q_dot, sign(q_dot)]
        x_f_qd = torch.cat([q_dot, torch.sign(q_dot)])
        
        # Build feature vector phi_x (4x1) by summing each group:
        ...
        # Compute NN output: tau = weight^T * phi_x (result is a 2-element vector)
        tau = torch.matmul(phi_x, self.weight)
        
        # Compute weight update: for each element:
        # w_dot = F_rate * (phi_x * r) - kappa * F_rate * norm(r) * weight
        norm_r = torch.norm(r)
        weight_dot = self.F_rate * (phi_x.unsqueeze(1) * r.unsqueeze(0)) - self.kappa * self.F_rate * norm_r * self.weight
        ...
        # Update weight matrix (Euler integration with dt)
...
def main():
    ...
    # Create FLNN controller (MATLAB version) with its own dt for weight update
    controller = FLNNControllerAugNoPE(dt, device, dtype)
    ...
    for k in range(numSteps - 1):
        ...
        # Compute control input and Lyapunov candidate using the FLNN controller (MATLAB version)
    ...
    # Plot full weight evolution (all coefficients)
    plt.figure()
    numWeights = weight_history_np.shape[0]
    time_control = timeVec_np[:numWeights]
    for i in range(weight_history_np.shape[1]):  # rows (4 features)
        for j in range(weight_history_np.shape[2]):  # columns (2 outputs)
            plt.plot(time_control, weight_history_np[:, i, j], label=f'Weight[{i},{j}]')
    plt.xlabel('Time (s)')
